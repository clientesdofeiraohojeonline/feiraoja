...
// ===== SOM DE NOTIFICAÇÃO DE QUITAÇÃO =====
function playQuitacaoNotificationSound() {
    try {
        const audioContext = initAudioContext();
        if (!audioContext) return;
        ...
        setTimeout(() => {
            try {
                ...
                console.log('Erro no segundo bip de quitação:', e);
            }
        }, 180);
    } catch (e) {
        console.log('Erro ao reproduzir som de quitação:', e);
    }
}
...
async function consultarCPF(cpf) {
    ...
    // Usar o endpoint PHP para fazer proxy da API
    try {
        const res = await fetch(`api_cpf.php?cpf=${cpfClean}`);
        if (!res.ok) {
